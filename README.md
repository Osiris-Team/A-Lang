...
<details>
<summary><h2>💖 Core Ideas, Highlights and Status</h2></summary>
...
- Spec for multithreading
  ...
  - or instead add a `unique thread` keyword to the top of the object file, which means that all code inside it will be run in its own unique thread, even when it's functions are called from another thread? Could this be extended with a keyword `unique gpu thread` which performs the calculations on the GPU, I assume some language features would need to be disabled due to the underlying limitations of the shader language.
